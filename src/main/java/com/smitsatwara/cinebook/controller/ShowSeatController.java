...
public class ShowSeatController {
    ...
    @GetMapping("/{showId}")
    public ResponseEntity<List<ShowSeatResponse>> getAllSeats(@PathVariable Long showId) {
        return ResponseEntity.ok(showSeatService.getShowSeats(showId));
    }
    @GetMapping("/{showId}/available")
    public ResponseEntity<List<ShowSeatResponse>> getAvailableSeats(@PathVariable Long showId) {
